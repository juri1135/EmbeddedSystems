...
{   
    int readIndex = 0;
    int totalReads = 0;
    char nameToConsume[5][16] = {"Alice", "Bob", "Carol", "David", "Erwin"};
    long valueToConsume[5] = {1.23, 4.56, 7.89, 0.12, 3.45};
    messageStruct nextConsumed;

    while (totalReads < TOTAL_COMM) {
        //못 들어가면 wait
        while (bufferCount == BUFFER_SIZE) ; /* do nothing */

        strcpy(nextConsumed.name, nameToConsume[readIndex%5]);
        nextConsumed.value = valueToConsume[readIndex%5];
